...
impl<T, A: AllocRef + Clone> RawTable<T, A> {
    ...
    fn fallible_with_capacity(
        ...
        Ok(Self {
            table: RawTableInner::fallible_with_capacity::<T>(alloc, capacity, fallability)?,
            marker: PhantomData,
        })
    }
    ...
    pub fn shrink_to(&mut self, min_size: usize, hasher: impl Fn(&T) -> u64) {
        ...
        if min_buckets < self.buckets() {
            ...
            if self.table.items == 0 {
                *self = Self::with_capacity(self.table.alloc.clone(), min_size)
            } else {
                // Avoid `Result::unwrap_or_else` because it bloats LLVM IR.
                if self
...
impl<A: AllocRef + Clone> RawTableInner<A> {
    #[cfg_attr(feature = "inline-more", inline)]
    unsafe fn new_uninitialized(
        alloc: A,
        buckets: usize,
    ...
    fn fallible_with_capacity<T>(
        ...
        if capacity == 0 {
            ...
            unsafe {
                ...
                Self::fallible_with_capacity_inner(alloc, buckets, fallability, layout, ctrl_offset)
            }
        }
    }
    ...
    unsafe fn fallible_with_capacity_inner(
        ...
        let result = Self::new_uninitialized(alloc, buckets, fallability, layout, ctrl_offset)?;
...
impl<'a, A: AllocRef + Clone> RawIterHashInner<'a, A> {
    #[inline]
